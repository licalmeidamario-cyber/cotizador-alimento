...
- **Impresión a PDF** (2026-09-09): se agregó `@page { size: letter; margin: 12mm; }` y CSS compacto en `@media print` (h1 19px, body 11px, tablas 10.5px, celdas `padding:3px 6px`, párrafos `margin:2px 0 !important`) para que cotización y análisis salgan en **1 sola hoja tamaño carta**. `imprimir(modalidad)` ahora fija `document.title` a `cotizacion alimento (CLIENTE FECHA)` (fecha ISO `YYYY-MM-DD`; sin cliente usa `sin cliente`) antes de `window.print()` y lo restaura después (esto define el nombre del PDF). Verificado con `Page.printToPDF` + `pdfinfo`: 1 página letter en ambos formatos. Fórmula del nombre capturada en el hilo de print.
