...
- Repo GitHub: **licalmeidamario-cyber/cotizador-alimento** (rama `main`). ⚠ **PÚBLICO** desde v1.4.0 (necesario para que la app detecte y descargue actualizaciones sin autenticación; no contiene secretos, los precios son configurables).
...
- **Renglón móvil/APK (v1.4.0)**: la fila de ingredientes usa `grid-cols-ings = minmax(0,1fr) minmax(0,74px) minmax(0,78px) auto` (definido en `tailwind.config.cjs`); el input de nombre ya NO usa `col-span-3`. Resultado: nombre, kilos, precio y × quedan en UNA misma fila a 375 px, sin desbordamiento horizontal (verificado con CDP).

- **Auto-actualización (v1.4.0)**:
  - `APP_VERSION = '1.4.0'` en `index.html` (regEX: `/const APP_VERSION = 'X'`). Al cargar, `revisarActualizacion()` consulta `https://api.github.com/repos/licalmeidamario-cyber/cotizador-alimento/releases/latest` (solo si online; 8 s de timeout; silencioso offline). Si hay versión mayor y no fue ignorada en la sesión, muestra banner inferior con [Actualizar] [Ahora no].
  - **Escritorio (Electron)**: `desktop/main.cjs` (nuevo) + `desktop/preload.js` exponen `window.desktopUpdater.installUpdate()`, `.reloadWindow()` y `.relaunchWindow()`. Descarga el asset **`www-<versión>.html`** de la release, valida que su `APP_VERSION` == tag, y lo coloca en `<userData>/www/index.html` (userData = `~/.config/cotizador-bigpickle-desktop` en Linux, `%APPDATA%\cotizador-bigpickle-desktop` en Windows). Al arrancar, main.cjs usa el contenido de `userData/www` SI existe, si no el empaquetado — así las actualizaciones son solo de contenido (un archivo). Sigue redirects (HTTP 302) y tiene timeout de descarga (90 s). Tras aplicar la actualización la app hace **`app.relaunch()` + `app.exit(0)`** (reinicio completo, el banner NO puede sobrevivir). `app.requestSingleInstanceLock()` evita instancias duplicadas (zombis que causaban confusión). Verificado E2E en Linux: app 1.4.0 + release ficticia 1.5.0 → banner "Nueva versión v1.5.0 (tienes v1.4.0)" → Actualizar → reinicio → v1.5.0, chip actualizado, banner ausente.
  - **UI**: header muestra chip **`vX.Y.Z`** (`#ver-chip`, texto `v` + APP_VERSION) y `#ver-msg` ("Actualizado (v1.4.0)"). El recordatorio "Ahora no" persiste en `localStorage['upd_ignorada']` con lógica **consciente de versión**: solo re-aparece si la versión nueva es estrictamente mayor que la ignorada.
  - ⚠ **BUG CRÍTICO RESUELTO (causa del "banner que nunca se quitaba")**: el banner usaba `class="upd-banner hidden"` y se mostraba/ocultaba con `classList.add/remove('hidden')`. PERO `.upd-banner { display:flex }` gana frente a la clase `.hidden { display:none }` de Tailwind según el orden de la hoja de estilos → el banner estaba SIEMPRE visible (computed `display:flex`), sin importar el estado de versión, con su texto estático. **Nunca esconder a base de la clase `hidden` de Tailwind sobre un elemento que ya lleva otra clase que fija `display`** (sea custom o utilidad `.flex`); el orden es frágil al inyectar CSS. SOLUCIÓN: el banner arranca con `style="display:none"` inline y el JS alterna `banner.style.display = 'flex'` / `'none'` (el estilo inline SIEMPRE gana). Verificado E2E con `getComputedStyle` (comprobar el estilo computado, NO `className`): al día → `display:none`; con v1.9.0 disponible → `display:flex` + texto "Nueva versión v1.9.0 (tienes v1.4.0)"; tras Actualizar → reinicio + `display:none` + chip v1.9.0.
  - Al descargar, si falla se **restaura el texto** "Actualización disponible" y se reactiva el botón (antes quedaba pegado en "Descargando…"). El descriptor de versión en `main.cjs` valida con regex `/const APP_VERSION\s*=\s*'([0-9.]+)'/` → solo versiones numéricas (para pruebas conviene tags como `v1.9.0`, NO con letras).
  - **Android (APK)**: no puede auto-reemplazar el APK (límite del sistema). El banner abre la página de release en el navegador del sistema vía **`@capacitor/browser`** (plugin oficial) → el usuario instala el APK nuevo. Sin banner si no hay internet.
  - **Flujo de release (cada versión nueva)**: 1) subir `APP_VERSION` en `index.html`; 2) `./build-css.sh`; 3) APK: `npx cap sync android && cd android && ./gradlew assembleDebug`, copiar a `~/Descargas/CotizadorBigPickle-v3.apk`; 4) desktop: `npm run build:win` y `npm run build:linux`, empaquetar portable tar.gz; 5) `gh release create vX.Y.Z <artefactos> www-X.Y.Z.html` (Ó actualizar la release existente con `--clobber`). El asset `www-<ver>.html` es OBLIGATORIO para el auto-update.
